...
import BackedCommunityNFTMockABI from "../../contracts/out/BackedCommunityNFTMock.sol/BackedCommunityNFTMock.json";
...
export async function proposeTx(
  changeProposals: XPChangeProposal[]
): Promise<number> {
  ...
  const iface = new ethers.utils.Interface(BackedCommunityNFTMockABI.abi);

  const traitScoreChangesParam = changeProposals.map((proposal) => [
  ...
  const transaction: SafeTransactionDataPartial = {
    ...
    data: iface.encodeFunctionData(
      "setCategoryScores(tuple(address addr, uint256 traitId, uint256 value)[])",
      [traitScoreChangesParam]
    ),
    value: "0",
  };
